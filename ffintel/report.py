...
function detail(id){
  ...
  <div class="kv">
   <div><span>Snap %</span><b>${pct(p.snap_pct)}</b></div><div><span>Target share</span><b>${pct(p.target_share)}</b></div>
...
const views={
 ...
 waivers(){
  ...
  ${table(L.waivers,[{k:'name',h:'Player',l:1,f:nm},{k:'score',h:'Score',f:x=>f1(x.score)},{k:'gain_week',h:'+Lineup/wk',f:x=>sgn(x.gain_week)},{k:'proj_ppg',h:'Proj',f:x=>f1(x.proj_ppg)},{k:'pct_owned',h:'Own%',f:x=>x.pct_owned==null?'–':f0(x.pct_owned)+(x.pct_change?` <span class="${x.pct_change>0?'good':'bad'}">${sgn(x.pct_change)}</span>`:'')},{k:'snap_pct',h:'Snap%',f:x=>pct(x.snap_pct)},{k:'drop',h:'Drop',l:1},{k:'why',h:'Why',l:1,f:x=>`<div class="why">${esc(x.why)}</div>`}],{})}</div>
  <div class="card"><h2>Best available by position</h2>${['QB','RB','WR','TE'].map(ps=>{const r=P.filter(p=>p.position===ps&&!p.owner_name&&p.ros_games>0).slice(0,5);return `<p style="margin:8px 0 2px"><b>${ps}</b></p>`+r.map(p=>`<div class="p" style="cursor:pointer" onclick="detail('${p.player_id}')">${nm(p)} <span class="mute">${f1(p.proj_ppg)} proj · ${ps}${f0(p.own_pos_rank)}</span></div>`).join('')}).join('')}</div>`;
 },
 trades(){
  if(!L)return noLeague();
...
const TABS=[['team','My team'],['waivers','Waivers'],['trades','Trades'],['value','Value board'],['rankings','Rankings'],['injuries','Injuries'],['method','How it works']];
